...
INTEGER, ALLOCATABLE :: NBSETLEV (:)          !Meta data
INTEGER, ALLOCATABLE :: VSETUVS (:), VSETS (:) !Meta data
...
CALL INV_TRANS_FIELD_API (US=[B(YLU), B(YLU1)], SPVORS=[B(YLSPVOR), B(YLSPVOR1)], &
                        & VS=[B(YLV), B(YLV1)], SPDIVS=[B(YLSPDIV), B(YLSPDIV1)], &
...
PRINT*,"CALL 2"
CALL INV_TRANS_FIELD_API (US=US, VS=VS, SPVORS=SPVORS, SPDIVS=SPDIVS, VSETUVS=VSETUVS, &
                        & SCALARS=SCALARS, SPSCALARS=SPSCALARS, VSETS=VSETS, LDACC=LLACC)
...
CALL INV_TRANS_FIELD_API (US=US, VS=VS, SPVORS=SPVORS, SPDIVS=SPDIVS, VSETUVS=VSETUVS, &
                       & SCALARS=SCALARS, SPSCALARS=SPSCALARS, VSETS=VSETS, &
		       & LDACC=LLACC, FSPGL_PROC=FSPGL_PROC)
