...
SUBROUTINE PRINT_DEBUG_INV_TRANS_FIELD_API(ZSPVOR, ZSPDIV, PSPSC2, PGPUV, PGP2, SPVORS,SPDIVS,SPSCALARS, &
...
REAL(KIND=JPRB),ALLOCATABLE :: PSPSC2(:,:)                       ! Spectral scalar fields (in)
REAL(KIND=JPRB),ALLOCATABLE :: PGPUV(:,:,:,:)                       ! Grid vector fields (out)
REAL(KIND=JPRB),ALLOCATABLE :: PGP2(:,:,:)                           ! Grid scalar fields (out)
...
TYPE(FIELD_BASIC_PTR),OPTIONAL  :: DSCALARSEW(:), DSCALARSNS(:) !Grid scalar fields derivatives NS(out)
...
CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_2D(PSPSC2, "PSPSC2",  DIMSPF)
CALL PRINT_DEBUG_FOOTER()
...
CALL PRINT_DEBUG_FIELDS(VORS,"VORS")
CALL PRINT_DEBUG_SUM(IFLDGUV)
ENDIF

IF (PRESENT(DIVS)) THEN
   CALL PRINT_DEBUG_FIELDS(DIVS,"DIVS")
   CALL PRINT_DEBUG_SUM(IFLDGUV)
ENDIF

IF (PRESENT(DUS))  THEN
  CALL PRINT_DEBUG_FIELDS(DUS, "DUS")
  CALL PRINT_DEBUG_SUM(IFLDGUV)
ENDIF

IF (PRESENT(DVS))  THEN
  CALL PRINT_DEBUG_FIELDS(DVS, "DVS")
  CALL PRINT_DEBUG_SUM(IFLDGUV)
ENDIF

CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_4D(PGPUV, "PGPUV",DIMGP4)
CALL PRINT_DEBUG_FOOTER()
...
IF (PRESENT(DSCALARSEW)) THEN
CALL PRINT_DEBUG_FIELDS(DSCALARSEW, "DSCALARSEW")
CALL PRINT_DEBUG_SUM(IFLDG)
ENDIF

IF (PRESENT(DSCALARSNS))  THEN
  CALL PRINT_DEBUG_FIELDS(DSCALARSNS,"DSCALARSNS")
  CALL PRINT_DEBUG_SUM(IFLDG)
ENDIF

CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_3D(PGP2,  "PGP2",DIMGP3)
CALL PRINT_DEBUG_FOOTER()
