MODULE PRINT_DEBUG_FIELD_API
  ...
  CONTAINS
SUBROUTINE PRINT_DEBUG_TABLE_HEADER(TABLE_NAME, DIM_NAMES)
  ...
  IF (PRESENT(DIM_NAMES)) THEN
    ...
    WRITE (*, "(A10,8X,A2, A7,A2)") &
            & TABLE_NAME, "| ", DIM_NAMES(1), " |" 
    ELSE IF (N_DIMS == 2 ) THEN
      WRITE (*, "(A10,8X,A2, A7,A3, A7,A2)") &
              & TABLE_NAME, "| ", DIM_NAMES(1), " | ", DIM_NAMES(2), " |" 
...
END SUBROUTINE PRINT_DEBUG_TABLE_HEADER
  ...
  SUBROUTINE PRINT_DEBUG_FIELDS(YLFL, FIELD_NAME, DIM_NAMES)
    ...
    CALL  PRINT_DEBUG_TABLE_HEADER(FIELD_NAME, DIM_NAMES)
...
SUBROUTINE PRINT_DEBUG_1D(A1, NAME, DIM_NAMES)
REAL(KIND=JPRB),INTENT(IN) :: A1(:)
CHARACTER(LEN=*), INTENT(IN) :: NAME
CHARACTER(LEN=*), OPTIONAL, INTENT(IN) :: DIM_NAMES(:)
CALL PRINT_DEBUG_TABLE_HEADER("", DIM_NAMES)

END SUBROUTINE PRINT_DEBUG_1D

SUBROUTINE PRINT_DEBUG_2D(A2, NAME, DIM_NAMES)
...
SUBROUTINE PRINT_DEBUG_3D(A3, NAME, DIM_NAMES)
...
SUBROUTINE PRINT_DEBUG_4D(A4, NAME, DIM_NAMES)
...
CHARACTER(LEN=7)            :: DIM_NAMES_SP_LEV(2), DIM_NAMES_SP_FIELDS(2), DIM_NAMES_GP_2(2), DIM_NAMES_GP_3(3), DIM_NAMES_GP_4(4)
...
DIM_NAMES_SP_LEV    = (/"NSPECS ", "NLEVS  "/)
DIM_NAMES_SP_FIELDS = (/"NSPECS ", "NFIELDS"/)
DIM_NAMES_GP_2      = (/"NPROMA ", "NGPBLKS"/)
DIM_NAMES_GP_3      = (/"NPROMA ", "NLEVS  ", "NGPBLKS"/)
DIM_NAMES_GP_4      = (/"NPROMA ", "NLEVS  ", "NFIELDS", "NGPBLKS"  /)

IF (PRESENT(SPVORS)) THEN
CALL PRINT_DEBUG_FIELDS(SPVORS, "SPVORS",DIM_NAMES_SP_LEV )
CALL PRINT_DEBUG_SUM(IFLDSUV)
ENDIF

IF (PRESENT(SPVORS)) THEN 
CALL PRINT_DEBUG_FIELDS(SPDIVS, "SPDIVS", DIM_NAMES_SP_LEV)
CALL PRINT_DEBUG_SUM(IFLDSUV)
ENDIF

CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_2D(ZSPVOR, "ZSPVOR", DIM_NAMES_SP_FIELDS)
...
CALL PRINT_DEBUG_FIELDS(SPSCALARS,"SPSCALARS", DIM_NAMES_SP_LEV)  
CALL PRINT_DEBUG_SUM(IFLDS)
ENDIF

CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_2D(ZSPSCALAR, "ZSPSCALAR",  DIM_NAMES_SP_FIELDS)
CALL PRINT_DEBUG_FOOTER()

CALL PRINT_DEBUG_HEADER("Output grid-point vector fields:", SIZE(US))

IF (PRESENT(US)) THEN
CALL PRINT_DEBUG_FIELDS(US,"US", DIM_NAMES_GP_3)
CALL PRINT_DEBUG_SUM(IFLDGUV)
ENDIF

IF (PRESENT(US))  THEN
CALL PRINT_DEBUG_FIELDS(US,"VS", DIM_NAMES_GP_3)
CALL PRINT_DEBUG_SUM(IFLDGUV)
ENDIF
...
CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_4D(ZGPUV, "ZGPUV",DIM_NAMES_GP_4)
CALL PRINT_DEBUG_FOOTER()

CALL PRINT_DEBUG_HEADER("Output grid-point scalar fields:", SIZE(SCALARS))
IF (PRESENT(SCALARS)) CALL PRINT_DEBUG_FIELDS(SCALARS, "SCALARS", DIM_NAMES_GP_3)
...
CALL PRINT_DEBUG_SEPARATOR(1)
CALL PRINT_DEBUG_3D(ZGP,  "ZGP",DIM_NAMES_GP_3)
CALL PRINT_DEBUG_FOOTER()
